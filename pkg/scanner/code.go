...
func ScanCode(targetDir string, rs rules.RuleSet) ([]Finding, error) {
	...
	for _, rule := range rs.Rules {
		// Target code rules
		if !strings.HasPrefix(rule.ID, "ISO-A8-28") {
			continue
		}
		...
		if matched {
			finding := Finding{File: filePath, Category: "code", Framework: "ISO-27001", Message: rule.Description}
			findings = append(findings, finding)
			fmt.Printf("[VIOLATION DETECTED] FLAW: %s\n", rule.Description)
			fmt.Println("  -> Framework: ISO-27001 (Secure Coding)")
		}
	}
	return findings, nil
}
